    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            {['Home', 'Rooms', 'Help', 'My Bookings'].map((item, index) => (
              <motion.div
                ...
                whileHover={{ y: -2 }}
              >
                <Link to={item === 'Home' ? '/' : `/${item.toLowerCase().replace(' ', '-')}`} className={`${item === 'Home' ? 'text-foreground font-medium' : 'text-muted-foreground'} hover:text-accent transition-colors`}>
                  {item}
                </Link>
              </motion.div>
            ))}
          ...
          {/* Right Section */}
          <motion.div className="hidden md:flex items-center gap-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.3 }}>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {currencies.map((curr) => (
                  <DropdownMenuItem key={curr.code} onClick={() => setCurrency(curr.code)} className="cursor-pointer">
            ...
            {user ? (
              ...
              <>
                ...
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button size="sm" className="bg-accent hover:bg-accent/90 text-accent-foreground" onClick={() => navigate('/signup')}>
                    Register
                  </Button>
                </motion.div>
              </>
                ...
                <div className="flex flex-col gap-4 px-4">
                  ...
                  {user ? (
                    <div className="space-y-2">
                      <Button variant="outline" size="sm" className="w-full" onClick={() => { navigate('/dashboard'); setMobileMenuOpen(false); }}>
                        <LayoutDashboard className="h-4 w-4 mr-2" />
                        Dashboard
                      </Button>
                      {isStaff && (
                        <Button variant="outline" size="sm" className="w-full" onClick={() => { navigate('/staff'); setMobileMenuOpen(false); }}>
                          <Users className="h-4 w-4 mr-2" />
                          Staff Portal
                        </Button>
                      )}
                      {isAdmin && (
                        <Button variant="outline" size="sm" className="w-full" onClick={() => { navigate('/admin'); setMobileMenuOpen(false); }}>
                          <Shield className="h-4 w-4 mr-2" />
                          Admin Panel
                        </Button>
                      )}
                      <Button variant="outline" size="sm" className="w-full text-destructive" onClick={() => { handleSignOut(); setMobileMenuOpen(false); }}>
                        <LogOut className="h-4 w-4 mr-2" />
                        Sign Out
                      </Button>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" className="flex-1" onClick={() => { navigate('/login'); setMobileMenuOpen(false); }}>Sign In</Button>
                      <Button size="sm" className="flex-1 bg-accent hover:bg-accent/90 text-accent-foreground" onClick={() => { navigate('/signup'); setMobileMenuOpen(false); }}>Register</Button>
                    </div>
                  )}
                </div>
              </motion.div>
